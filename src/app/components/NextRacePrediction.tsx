...
export function NextRacePrediction() {
  ...
  if (!upcomingRace) {
    return (
      <section className="rounded-[28px] border border-slate-200/70 bg-white/85 p-8 shadow-sm">
        <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500">Predictions</p>
        <h2 className="mt-2 text-2xl font-semibold text-slate-900">Next race outlook unavailable</h2>
    ...
    <section className="space-y-6">
      <div className="rounded-[28px] border border-slate-200/70 bg-white/85 p-6 shadow-sm backdrop-blur">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500">Predictions</p>
            <h2 className="mt-2 text-2xl font-semibold text-slate-900">Next race win probability</h2>
          ...
          <div className="rounded-[24px] border border-slate-200/70 bg-slate-50 px-5 py-4 text-sm text-slate-600">
            <p className="font-semibold text-slate-900">{upcomingRace.name}</p>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.7fr_1fr]">
        <article className="rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b border-slate-200/70 pb-4">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Projected race leaderboard</h3>
          ...
          {loading ? (
            ...
            <div className="mt-6 rounded-[24px] border border-dashed border-slate-300 bg-slate-50 p-8 text-sm text-slate-500">
              No drivers match the current filters.
            </div>
          ) : (
            <div className="mt-6 space-y-4">
              {rows.map((row, index) => (
                <div key={row.id} className="rounded-[24px] border border-slate-200/70 bg-slate-50/80 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                    <div className="flex min-w-0 flex-1 items-center gap-4">
                      ...
                      <div
                        className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border border-slate-200"
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="rounded-full bg-slate-900 px-2 py-0.5 text-xs font-medium text-white">#{row.number}</span>
                        </div>
                        <p className="mt-1 text-sm text-slate-600">{row.team}</p>
                    ...
                    <div className="w-full lg:w-[22rem]">
                      <div className="mb-2 flex items-center justify-between text-sm">
                        <span className="font-medium text-slate-700">Win probability</span>
                        <span className="font-semibold text-slate-900">{row.probability.toFixed(1)}%</span>
                      </div>
        ...
        <div className="space-y-6">
          <article className="rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-semibold text-slate-900">Circuit read</h3>
            </div>
            <p className="mt-4 text-sm leading-relaxed text-slate-600">{upcomingRace.summary}</p>
            <p className="mt-4 rounded-[20px] bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">{upcomingRace.modelFocus}</p>
            {latestRace && (
              <div className="mt-4 rounded-[20px] border border-slate-200/70 bg-white p-4 text-sm text-slate-600">
                <p className="font-semibold text-slate-900">Latest data feed</p>
          ...
          <article className="rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Model inputs</h3>
            ...
            <p className="mt-4 text-xs leading-relaxed text-slate-500">
              This is a transparent heuristic model for dashboard insight, not betting odds or a trained machine-learning forecast.
            </p>
          </article>
        </div>
      </div>
